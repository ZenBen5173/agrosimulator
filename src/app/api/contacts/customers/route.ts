...
export async function GET(request: Request) {
  ...
  const { data } = await supabase.from("customers").select("*").eq("farm_id", farmId).order("name");
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.from("customers").insert({ farm_id, name, phone, address, notes }).select().single();
