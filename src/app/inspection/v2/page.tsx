...
function ResultStep({
  ...
  return (
    <section className="space-y-4">
      ...
      {session && <DownloadPdfButton session={session} />}
 ...
 * session to /api/diagnosis/v2/pdf, gets back an application/pdf blob,
 * triggers a browser download with the case-reference filename. Loading
 * + error states inline so the farmer never gets a silent failure.
 */
function DownloadPdfButton({ session }: { session: DiagnosisSession }) {
